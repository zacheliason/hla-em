...
def group_by_protein(df):
	...
	expanded_columns.columns = ['HLAletter', 'group', 'protein', 'coding', 'noncoding']

	# Fill any None values with Nones
	expanded_columns = expanded_columns.where(expanded_columns.notnull(), None)

	# Concatenate the expanded columns with the original DataFrame
	df = pd.concat([df, expanded_columns], axis=1)
	df = df.sort_values(by=['MLE_Reads', 'HLA_code'], ascending=[False, True])

	grouped = df.groupby(['HLAletter', 'group', 'protein']).agg({
		'HLAtype': 'first',
		'HLA_type': 'first',
		'HLA_code': 'first',
		'MappedReads': 'sum',
		...
		'MLE_Reads': 'sum',
		'MLE_Probability': 'sum'
	}).reset_index().sort_values(by='MLE_Reads', ascending=False).drop(columns=['group', 'protein'])
	...
	for trial_dir in output_dirs:
		...
		if tool == "HLA_EM":
			...
			if len(final_predictions_path) == 0:
				print('Attempting to clean ' + trial_dir)
				results_path = [os.path.join(trial_dir, x) for x in trial_results if x.endswith("results.tsv")]
				if len(results_path) == 0:
					print(f"Error: {trial_dir} does not contain a results.tsv file")
					continue
				else:
					results_path = results_path[0]

				clean_output(results_path)
...
# scores = score_output("/Users/zacheliason/Downloads/output_zipped_copy/output_single", "/Users/zacheliason/Downloads/hla-em/reference/allele_record.csv", tool="Optitype")
# print(scores[['allele_group_score', 'protein_score', 'coding_score', 'noncoding_score']].mean())
# clean_output("/Users/zacheliason/Downloads/output_zipped_copy/output_single/trial_4/trial_4.results.tsv")
